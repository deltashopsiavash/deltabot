...
if(isset($text) && is_string($text)){
    ...
    if(($from_id == $admin || (($userInfo['isAdmin'] ?? false) == true)) && (empty($data) || $data === null)){
        ...
        // Plans
        if($t0 === 'پلن های نمایندگی' || $t0 === '📦 پلن های نمایندگی' || $t0 === 'مدیریت پلن های نمایندگی' || $t0 === 'پلن‌های نمایندگی'){
            setUser('none','step');
            setUser('', 'temp');
            $data = 'adminResellerPlans';
        }
        if($t0 === 'افزودن پلن' || $t0 === '➕ افزودن پلن' || $t0 === '➕ افزودن پلن نمایندگی' || $t0 === 'افزودن پلن نمایندگی'){
            setUser('none','step');
            setUser('', 'temp');
        ...
        if($t0 === '➕ ساخت ربات جدید' || $t0 === '➕ ساخت ربات'){
            setUser('none','step');
            setUser('', 'temp');
...
if(($from_id == $admin || ($userInfo['isAdmin'] ?? false) == true) && (empty($data) || $data === null)){
    ...
    if($t === '🔙 بازگشت به پنل مدیریت' || $t === 'بازگشت به پنل مدیریت' || $t === '😪 منصرف شدم بیخیال' || $t === 'منصرف شدم بیخیال'){
        setUser('none','step');
        $data = 'adminMenu';
    }
}
...
if($data=='adminResellerBots' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keyboard = [
        'keyboard'=>[
            ...
            [['text'=>'➕ افزودن پلن نمایندگی']],
            [['text'=>'😪 منصرف شدم بیخیال']],
        ],
        'resize_keyboard'=>true
    ];

    sendMessage("🤖 مدیریت ربات ها\n\nیکی از گزینه‌ها را انتخاب کنید:", json_encode($keyboard));
    exit;
}


// ===== Admin Reseller Plans =====
if($data=='adminResellerPlans' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ensureResellerTables();
    ...
    $ik = [
        'inline_keyboard'=>[
            ...
            [['text'=>'🔙 بازگشت','callback_data'=>'adminResellerBots']]
        ]
    ];
...
if($data=='resellerPlanToggleMenu' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $rows[]=[[ 'text'=>$buttonValues['cancel'], 'callback_data'=>"adminResellerPlans" ]];
...
if(preg_match('/^adminResBot_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    if(!$b){
        smartSendOrEdit($message_id,"❌ پیدا نشد.",['inline_keyboard'=>[[['text'=>$buttonValues['back'],'callback_data'=>"adminResBotsList_0"]]]]);
    }else{
...
if(preg_match('/^adminResBotDelete_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    smartSendOrEdit($message_id,"✅ حذف شد.",['inline_keyboard'=>[[['text'=>$buttonValues['back'],'callback_data'=>"adminResBotsList_0"]]]]);
}
