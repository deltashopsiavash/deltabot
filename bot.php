...
if(preg_match('/increaseWalletWithCartToCart(?<hashId>.*)/',$data, $match)) {
    ...
    sendMessage(str_replace(["ACCOUNT-NUMBER", "HOLDER-NAME"],[pickRandomValue($paymentKeys['bankAccount']),$paymentKeys['holderName']], $mainValues['increase_wallet_cart_to_cart']),$cancelKey, "HTML");
    exit;
}
...
foreach($adminsList as $adm){
    $tmpRes = sendPhoto($fileid, $msg,$keyboard, "HTML", $adm);
    if($res === null){
        $res = $tmpRes;
        $msgId = $tmpRes->result->message_id;
    }
}
...
if($userInfo['step'] == 's2a' and $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    sendMessage("پین بشه تو پی‌وی کاربر؟",json_encode(['inline_keyboard'=>[
    [['text'=>"بله پین کن",'callback_data'=>"pinSend2All" . $id],['text'=>"نه پین نکن",'callback_data'=>"noPinSend2All" . $id]]
    ]]));
}
...
if(preg_match('/payCustomWithCartToCart(.*)/',$data, $match)) {
    ...
    setUser($data);
    delMessage();
    sendMessage(str_replace(["ACCOUNT-NUMBER", "HOLDER-NAME"],[pickRandomValue($paymentKeys['bankAccount']),$paymentKeys['holderName']], $mainValues['buy_account_cart_to_cart']),$cancelKey, "HTML");
    exit;
}
...
// ===================== User Quick Actions (Admin) =====================
if(preg_match('/^userAct_(inc|dec|ban|unban|pm|disc|test|orders|exempt|reset)_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $act = $m[1];
    $uid = (int)$m[2];
    if($act == "ban"){
        $stmt = $connection->prepare("UPDATE `users` SET `step`='banned' WHERE `userid`=?");
    ...
    if($act == "unban"){
        $stmt = $connection->prepare("UPDATE `users` SET `step`='' WHERE `userid`=? AND `step`='banned'");
        $stmt->bind_param("i",$uid); $stmt->execute(); $stmt->close();
        alert("کاربر آزاد شد ✅");
        exit;
    }
    ...
    if($act == "orders"){
        $stmt = $connection->prepare("SELECT `id`,`amount`,`date` FROM `orders_list` WHERE `userid`=? ORDER BY `id` DESC LIMIT 10");
        $stmt->bind_param("i",$uid);
        $stmt->execute();
        $r = $stmt->get_result();
        $stmt->close();
        $txt = "🧾 ۱۰ سفارش آخر کاربر <code>$uid</code>\n\n";
        if($r->num_rows==0){
            $txt .= "سفارشی ثبت نشده است.";
        }else{
            while($row=$r->fetch_assoc()){
                $txt .= "• #".$row['id']." | ".number_format($row['amount'])." تومان | ".$row['date']."\n";
            }
        }
        sendMessage($txt, json_encode(['inline_keyboard'=>[[['text'=>$buttonValues['back_button'],'callback_data'=>"userReports"]]]]), "HTML");
        exit;
    }
    // actions requiring input
    if($act == "inc" || $act == "dec"){
        setUser("userAct_" . $act . "_" . $uid);
        sendMessage("مبلغ را فقط عدد وارد کنید (تومان)\nکاربر: <code>$uid</code>", $cancelKey, "HTML");
        exit;
    }
    if($act == "pm"){
        setUser("userAct_pm_" . $uid);
        sendMessage("متن پیام خصوصی را ارسال کنید\nکاربر: <code>$uid</code>", $cancelKey, "HTML");
        exit;
    }
    if($act == "disc"){
        setUser("userAct_disc_" . $uid);
        sendMessage("درصد تخفیف کاربر را فقط عدد وارد کنید (مثلاً 10)\nکاربر: <code>$uid</code>", $cancelKey, "HTML");
        exit;
    }
    if($act == "test"){
        setUser("userAct_test_" . $uid);
        sendMessage("محدودیت اکانت تست را فقط عدد وارد کنید\nکاربر: <code>$uid</code>", $cancelKey, "HTML");
        exit;
    }
    if($act == "exempt"){
        // toggle exempt flag in setting
        $key = "USER_EXEMPT_AUTOACCEPT_" . $uid;
        $stmt = $connection->prepare("SELECT `value` FROM `setting` WHERE `type`=?");
        ...
        alert("استثنا تایید خودکار: $val");
        exit;
    }
}
if(preg_match('/^userAct_(inc|dec)_(\d+)/',$userInfo['step'],$m) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $act = $m[1];
    $uid = (int)$m[2];
    if(!is_numeric($text)){
        sendMessage("فقط عدد ارسال کنید", $cancelKey);
        exit;
    }
    $amount = (int)$text;
    if($amount<=0){
        sendMessage("عدد معتبر وارد کنید", $cancelKey);
        exit;
    }
    if($act=="inc"){
        $stmt = $connection->prepare("UPDATE `users` SET `wallet` = `wallet` + ? WHERE `userid`=?");
    ...
    $stmt->bind_param("ii",$amount,$uid); $stmt->execute(); $stmt->close();
    setUser();
    sendMessage("✅ انجام شد برای کاربر <code>$uid</code>", null, "HTML");
    exit;
}
if(preg_match('/^userAct_pm_(\d+)/',$userInfo['step'],$m) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid=(int)$m[1];
    sendMessage($text, null, null, $uid);
    setUser();
    sendMessage("✅ پیام ارسال شد به <code>$uid</code>", null, "HTML");
    exit;
}
if(preg_match('/^userAct_(disc|test)_(\d+)/',$userInfo['step'],$m) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $field=$m[1]; $uid=(int)$m[2];
    if(!is_numeric($text)){
        sendMessage("فقط عدد ارسال کنید", $cancelKey);
        exit;
    }
    $val = (int)$text;
    $key = ($field=="disc"?"USER_DISCOUNT_":"USER_TEST_LIMIT_") . $uid;
    $stmt = $connection->prepare("SELECT `id` FROM `setting` WHERE `type`=?");
    $stmt->bind_param("s",$key); $stmt->execute(); $res=$stmt->get_result(); $stmt->close();
    if($res->num_rows>0){
        $stmt=$connection->prepare("UPDATE `setting` SET `value`=? WHERE `type`=?");
        $stmt->bind_param("ss",$val,$key);
    }else{
    ...
    $stmt->execute(); $stmt->close();
    setUser();
    sendMessage("✅ ذخیره شد برای کاربر <code>$uid</code>", null, "HTML");
    exit;
}
