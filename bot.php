...
/**
 * User action panel (after userReports -> getUserInfoKeys)
 */
if(preg_match('/^uInc(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    delMessage();
    sendMessage($mainValues['enter_increase_amount'],$cancelKey);
    setUser("uInc_" . $m[1]);
}
if(preg_match('/^uDec(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    delMessage();
    sendMessage($mainValues['enter_decrease_amount'],$cancelKey);
    setUser("uDec_" . $m[1]);
}
if(preg_match('/^uBan(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = (int)$m[1];
...
if(preg_match('/^uUnban(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = (int)$m[1];
...
if(preg_match('/^uZero(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = (int)$m[1];
...
if(preg_match('/^uPm(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    delMessage();
    sendMessage("پیام خصوصی را ارسال کنید",$cancelKey);
    setUser("uPm_" . $m[1]);
}
if(preg_match('/^uDisc(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    delMessage();
    sendMessage("درصد تخفیف کاربر را ارسال کنید (مثلا 10)",$cancelKey);
    setUser("uDisc_" . $m[1]);
}
if(preg_match('/^uTest(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = (int)$m[1];
    // toggle: if freetrial is null -> set blocked, else -> null
    $stmt = $connection->prepare("SELECT `freetrial` FROM `users` WHERE `userid`=?");
    $stmt->bind_param("i",$uid);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    $cur = $row['freetrial'] ?? null;
    $new = is_null($cur) ? "blocked" : null;
    $stmt = $connection->prepare("UPDATE `users` SET `freetrial`=? WHERE `userid`=?");
    $stmt->bind_param("si",$new,$uid);
    $stmt->execute(); $stmt->close();
    alert(is_null($new) ? "✅ محدودیت تست برداشته شد" : "✅ تست برای کاربر محدود شد");
}
if(preg_match('/^uOrders(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = (int)$m[1];
    ...
    if($res->num_rows == 0){
        ...
        $out = "🧾 آخرین سفارش‌های کاربر:\n";
        while($r = $res->fetch_assoc()){
            $out .= "• #".$r['id']." | ".$r['name']." | ".$r['price']."\n";
        }
        sendMessage($out,null,"HTML");
    }
}
if(preg_match('/^uExAuto(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = (int)$m[1];
    $new = toggleUserTempFlag($uid, 'exclude_auto_cart');
    alert($new ? "✅ از تایید خودکار مستثنا شد" : "✅ استثنا برداشته شد");
}

if(preg_match('/^uInc_(\d+)/',$userInfo['step'],$m) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $stmt->execute(); $stmt->close();
    sendMessage("✅ مبلغ ".number_format($amount)." تومان اضافه شد",$removeKeyboard);
    sendMessage($mainValues['reached_main_menu'],getAdminKeys());
    setUser();
}
if(preg_match('/^uDec_(\d+)/',$userInfo['step'],$m) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    if(!is_numeric($text)){ sendMessage($mainValues['send_only_number']); exit(); }
    $uid = (int)$m[1];
    $amount = (int)$text;
    $stmt = $connection->prepare("UPDATE `users` SET `wallet`=`wallet`-? WHERE `userid`=?");
    $stmt->bind_param("ii",$amount,$uid);
    $stmt->execute(); $stmt->close();
    sendMessage("✅ مبلغ ".number_format($amount)." تومان کم شد",$removeKeyboard);
    sendMessage($mainValues['reached_main_menu'],getAdminKeys());
    setUser();
}
if(preg_match('/^uPm_(\d+)/',$userInfo['step'],$m) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = (int)$m[1];
    sendMessage($text,null,"HTML",$uid);
    sendMessage("پیامت به کاربر ارسال شد",$removeKeyboard);
    sendMessage($mainValues['reached_main_menu'],getAdminKeys());
    setUser();
}
if(preg_match('/^uDisc_(\d+)/',$userInfo['step'],$m) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    if(!is_numeric($text)){ sendMessage($mainValues['send_only_number']); exit(); }
    $uid = (int)$m[1];
    $percent = (int)$text;
    $stmt = $connection->prepare("UPDATE `users` SET `discount_percent`=? WHERE `userid`=?");
    $stmt->bind_param("si",$percent,$uid);
    $stmt->execute(); $stmt->close();
    sendMessage("✅ درصد تخفیف ثبت شد",$removeKeyboard);
    sendMessage($mainValues['reached_main_menu'],getAdminKeys());
    setUser();
}
...
if(preg_match('/increaseWalletWithCartToCart(?<hashId>.*)/',$data, $match)) {
    ...
    sendMessage(str_replace(["ACCOUNT-NUMBER", "HOLDER-NAME"],[pickRandomBankAccount($paymentKeys['bankAccount']),$paymentKeys['holderName']], $mainValues['increase_wallet_cart_to_cart']),$cancelKey, "HTML");
    exit;
}
...
if(preg_match('/^yesSend2All(\d+)/', $data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    // ask pin option first
    editText($message_id,'پیام در پی‌وی کاربران پین شود؟',json_encode(['inline_keyboard'=>[
        [['text'=>'بله ✅','callback_data'=>'pinSend2All1_' . $match[1]],['text'=>'خیر ❌','callback_data'=>'pinSend2All0_' . $match[1]]]
    ]]));
}
if(preg_match('/^pinSend2All(\d)_(\d+)/', $data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $pin = $match[1]; $id = $match[2];
    // store pin choice by suffixing type with _pin
    if($pin == "1"){
        $stmt = $connection->prepare("UPDATE `send_list` SET `type` = CONCAT(`type`,'_pin') WHERE `id` = ? AND `type` NOT LIKE '%_pin'");
...
if(preg_match('/payCustomWithCartToCart(.*)/',$data, $match)) {
    ...
    setUser($data);
    delMessage();
    sendMessage(str_replace(["ACCOUNT-NUMBER", "HOLDER-NAME"],[pickRandomBankAccount($paymentKeys['bankAccount']),$paymentKeys['holderName']], $mainValues['buy_account_cart_to_cart']),$cancelKey, "HTML");
    exit;
}
