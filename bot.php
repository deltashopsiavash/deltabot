...
if(($from_id == $admin || ($userInfo['isAdmin'] ?? false) == true) && (empty($data) || $data === null)){
    ...
    if($t === 'ربات ها🤖' || $t === 'ربات ها 🤖' || $t === '🤖 ربات ها' || $t === '🤖 ربات‌ها' || $t === '🤖 ربات‌ها' || $t === 'ربات‌ها' || $t === 'ربات ها' || $t === 'مدیریت ربات 🤖' || $t === 'مدیریت ربات ها 🤖' || $t === '🤖 مدیریت ربات ها' || $t === 'مدیریت ربات ها'){
    ...
    if($t === '🔙 بازگشت به پنل مدیریت' || $t === 'بازگشت به پنل مدیریت'){
        $data = 'adminMenu';
    }
}

function smartSendOrEdit($msgId, $txt, $keys){
    // If we have a callback context we can edit, otherwise send a new message.
    $res = null;
    if(!empty($GLOBALS['data'])){
        $res = editText($msgId, $txt, $keys);
        if(isset($res->ok) && $res->ok) return $res;
    }
    return sendMessage($txt, $keys);
}
...
if($data=='adminResellerBots' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    sendMessage("🤖 مدیریت ربات ها\n\nیکی از گزینه‌ها را انتخاب کنید:", $keyboard);
    exit;
}
