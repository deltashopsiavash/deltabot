...
function getUserPanelText($uid){
    $userDetail = bot('getChat',['chat_id'=>$uid])->result;
    $userUserName = $userDetail->username??"";
...
function sendUserPanel($uid){
    $keys = getUserInfoKeys($uid);
    if($keys == null){
        sendMessage("کاربری با این آیدی یافت نشد");
        return;
    }
    sendMessage(getUserPanelText($uid), $keys, "HTML");
}
function editUserPanel($uid, $msgId=null){
    global $message_id;
    ...
    editText($msgId, getUserPanelText($uid), $keys, "HTML");
}
...
if(preg_match('/^uIncWallet(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    setUser("increaseWalletUser" . $match[1]);
}
if(preg_match('/^uDecWallet(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    setUser("decreaseWalletUser" . $match[1]);
}
...
if(preg_match('/^uPm(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    setUser("uPmSend" . $match[1]);
}
if(preg_match('/^uPmSend(\d+)/',$userInfo['step'],$match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
...
if(preg_match('/^uDiscount(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    setUser("uDiscountSet" . $match[1]);
}
if(preg_match('/^uDiscountSet(\d+)/',$userInfo['step'],$match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
...
if(preg_match('/^uTestLimit(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    setUser("uTestLimitSet" . $match[1]);
}
if(preg_match('/^uTestLimitSet(\d+)/',$userInfo['step'],$match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
...
if(preg_match('/^uAuto(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $stmt->bind_param("s",$type);
    $stmt->execute();
    $val = $stmt->get_result()->fetch_assoc()['value']??"0";
    $stmt->close();
    $newVal = ($val=="1")?"0":"1";
...
if(preg_match('/increaseWalletWithCartToCart(.*)/',$userInfo['step'], $match) and $text != $buttonValues['cancel']){
    if(isset($update->message->photo)){
        ...
        // Auto-accept cart-to-cart (unless user is excluded)
        if(($botState['cartToCartAutoAcceptType']??"0")!="0" && !isUserNoAutoApprove($from_id)){
            ...
            $stmt = $connection->prepare("UPDATE `users` SET `wallet` = `wallet` + ? WHERE `userid` = ?");
            $stmt->bind_param("ii", $payInfo['price'], $from_id);
            $stmt->execute();
            $stmt->close();

            sendMessage("✅ افزایش موجودی شما به صورت خودکار تایید شد\nمبلغ " . number_format($payInfo['price']) . " تومان به کیف پول شما اضافه شد");
            exit();
        }
$msg = str_replace(['PRICE', 'USERNAME', 'NAME', 'USER-ID'],[$price, $username, $name, $from_id], $mainValues['increase_wallet_request_message']);
        ...
        foreach($adminIds as $aid){
            $r = sendPhoto($fileid, $msg,$keyboard, "HTML", $aid);
            if($aid == $admin) $resMain = $r;
        }
        $res = $resMain??$r;
        $msgId = $res->result->message_id;
...
if((preg_match('/^discountSelectPlan(\d+)_(\d+)_(\d+)/',$userInfo['step'],$match) || 
    ...
    if(preg_match('/^discountSelectPlan/', $userInfo['step'])){
        ...
        if($list->num_rows>0){
            ...
            if($canUse > $userUsedCount){
                ...
                $afterDiscount = applyUserPercentDiscount($uid, $afterDiscount);
    ...
    if($price == 0 or ($from_id == $admin)){
        ...
        if(!preg_match('/^discountSelectPlan/', $userInfo['step'])){
            ...
            $price = applyUserPercentDiscount($uid, $afterDiscount);
        }
...
if(preg_match('/^discountRenew(\d+)_(\d+)/',$userInfo['step'], $match) || preg_match('/renewAccount(\d+)/',$data,$match) && $text != $buttonValues['cancel']){
    ...
    if(!preg_match('/^discountRenew/', $userInfo['step'])){
        ...
        $rowId = $stmt->insert_id;
        $stmt->close();
    }else $price = applyUserPercentDiscount($uid, $afterDiscount);
