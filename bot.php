...
// Auto DB backup (daily) - sends to bot admin when they interact
if(($from_id == $admin || ($userInfo['isAdmin'] ?? false) == true)){
    $st = getSettingValue('AUTO_BACKUP_STATE', '{"enabled":0,"last":0}');
    $stj = json_decode($st, true);
    $enabled = (int)($stj['enabled'] ?? 0);
    $last = (int)($stj['last'] ?? 0);
    if($enabled && (time() - $last > 86400)){
...
// Periodic auto backup (main bot only)
autoBackupTick();
...
// ===== Reseller bot: Update (only in child bot for that bot admin) =====
if($data=='resellerBotUpdate' && $isChildBot && $from_id == $admin){
    // In this architecture, all reseller bots use the same source code.
    // This action re-runs lightweight maintenance so the bot uses the latest shared settings/DB schema.
    ensureResellerTables();
    resellerBotsMaintenance();
    alert("✅ بروزرسانی انجام شد و آخرین نسخه فعال است.");
}




    // ===== Admin Backup Menu (advanced) =====
    if($data=='adminBackupMenu' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
        $st = getSettingValue('AUTO_BACKUP_STATE', '{"enabled":0,"interval":1440,"last":0}');
        $stj = json_decode($st, true);
        if(!is_array($stj)) $stj = ['enabled'=>0,'interval'=>1440,'last'=>0];
        $enabled = (int)($stj['enabled'] ?? 0);
        $interval = (int)($stj['interval'] ?? 1440);
        if($interval < 1) $interval = 1;

        $keys = ['inline_keyboard'=>[
            [['text'=>'🧰 بکاپ دستی','callback_data'=>'adminBackupManual']],
            [['text'=>($enabled ? "✅ بکاپ خودکار: روشن ({$interval} دقیقه)" : "❌ بکاپ خودکار: خاموش"),'callback_data'=>'adminBackupAutoMenu']],
            [['text'=>'📤 افزودن بکاپ','callback_data'=>'adminBackupRestore']],
            [['text'=>$buttonValues['back'],'callback_data'=>'adminMenu']],
        ]];
        editText($message_id, "🗄 مدیریت بکاپ

• بکاپ دستی: همین الان فایل SQL ارسال می‌شود
• بکاپ خودکار: ارسال دوره‌ای به ادمین‌ها
• افزودن بکاپ: فایل SQL را ارسال کنید تا بازگردانی شود", $keys);
    }
    ...
    if($data=='adminBackupManual' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
        ...
        $sql = dbCreateSqlBackup();
        if(!$sql){
            alert('خطا در ساخت بکاپ', true);
        }else{
            $tmp = sys_get_temp_dir() . '/deltabot_backup_' . date('Ymd_His') . '.sql';
            file_put_contents($tmp, $sql);
            sendDocument($tmp, "🗄 بکاپ دیتابیس
".date('Y-m-d H:i:s'));
            @unlink($tmp);
        }
    }

    if($data=='adminBackupAutoMenu' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
        $st = getSettingValue('AUTO_BACKUP_STATE', '{"enabled":0,"interval":1440,"last":0}');
        $stj = json_decode($st, true);
        if(!is_array($stj)) $stj = ['enabled'=>0,'interval'=>1440,'last'=>0];
        $enabled = (int)($stj['enabled'] ?? 0);
        ...
        $keys = ['inline_keyboard'=>[
            [['text'=>($enabled?"✅ روشن":"❌ خاموش"),'callback_data'=>'adminBackupAutoToggle']],
            [['text'=>"⏱ تنظیم بازه: {$interval} دقیقه",'callback_data'=>'adminBackupAutoSetInterval']],
            [['text'=>$buttonValues['back'],'callback_data'=>'adminBackupMenu']],
        ]];
        editText($message_id, "⏱ بکاپ خودکار

- اگر روشن باشد، هر X دقیقه یک فایل بکاپ به همه ادمین‌ها ارسال می‌شود.", $keys);
    }
    ...
    if($data=='adminBackupAutoToggle' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
        ...
        $data = 'adminBackupAutoMenu';
    }

    if($data=='adminBackupAutoSetInterval' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
        setUser('awaiting_backup_interval','step');
        editText($message_id, "⏱ هر چند دقیقه یک بکاپ ارسال شود؟

فقط عدد ارسال کنید (مثال: 2)", ['inline_keyboard'=>[[['text'=>$buttonValues['back'],'callback_data'=>'adminBackupAutoMenu']]]]);
    }

    if(($userInfo['step'] ?? '') == 'awaiting_backup_interval' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
        if(isset($update->message) && isset($update->message->text)){
            $min = trim($update->message->text);
            if(is_numeric($min)){
                $min = (int)$min;
                if($min < 1) $min = 1;
                $st = getSettingValue('AUTO_BACKUP_STATE', '{"enabled":0,"interval":1440,"last":0}');
                ...
                if(!is_array($stj)) $stj = ['enabled'=>0,'interval'=>1440,'last'=>0];
                $stj['interval'] = $min;
                $stj['enabled'] = 1; // enable when interval is set
                upsertSettingValue('AUTO_BACKUP_STATE', json_encode($stj, JSON_UNESCAPED_UNICODE));
                setUser('none','step');
                sendMessage("✅ انجام شد. از این به بعد هر {$min} دقیقه بکاپ ارسال می‌شود.");
                // show menu again
                $data = 'adminBackupAutoMenu';
            }else{
