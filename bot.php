...
// Auto DB backup (interval) - sends to main bot admin
if(!isset($isChildBot) || !$isChildBot){
    $st = getSettingValue('AUTO_BACKUP_STATE', '{"enabled":0,"interval":2,"last":0}');
    $stj = json_decode($st, true);
    if(!is_array($stj)) $stj = ["enabled"=>0,"interval"=>2,"last"=>0];
    $enabled = (int)($stj['enabled'] ?? 0);
    $interval = (int)($stj['interval'] ?? 2);
    if($interval < 1) $interval = 1;
    $last = (int)($stj['last'] ?? 0);

    if($enabled && (time() - $last >= ($interval * 60))){
        $sql = dbCreateSqlBackup();
        if($sql){
            $tmp = sys_get_temp_dir() . '/deltabot_auto_backup_' . date('Ymd_His') . '.sql';
            file_put_contents($tmp, $sql);
            // send to main admin chat_id
            sendDocument($tmp, "🗄 بکاپ خودکار دیتابیس\n".date('Y-m-d H:i:s'), null, "HTML", (int)$admin);
            @unlink($tmp);
            $stj['last'] = time();
            upsertSettingValue('AUTO_BACKUP_STATE', json_encode($stj, JSON_UNESCAPED_UNICODE));
        }
    }
}
    ...
    if(isset($update->message) && isset($update->message->document)){
        ...
        if($fileId){
            ...
            if($sql !== false){
                ...
                sendMessage("❌ خطا در دانلود فایل از تلگرام.");
            }
        }
    }else{
...
⚠️توجه کنید⚠️ ایدی عددی باید فقط عدد باشه و هیج چیز اضافه دیگه ای نباشه");
    ...
    if(preg_match('/^resellerAwaitAdmin_(\d+)$/', $userInfo['step'] ?? '', $mm)){
        ...
        $row = $connection->query("SELECT `bot_token`,`expires_at`,`bot_username`,`admin_userid` FROM `reseller_bots` WHERE `id`={$rid} LIMIT 1")->fetch_assoc();
        if($row && !empty($row['bot_token'])){
            botWithToken($row['bot_token'], "setWebhook", ['url'=>$hookUrl]);
        }

        setUser("none", "step");
        $exp = jdate('Y/m/d H:i', (int)$row['expires_at']);
        $uname = $row['bot_username'] ? '@'.$row['bot_username'] : '---';
...
if($data=='adminResellerBots' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keys = ['inline_keyboard'=>[
        ...
        [['text'=>"🧾 پلن ها",'callback_data'=>"adminResPlans"]],
        [['text'=>"➕ ساخت ربات",'callback_data'=>"adminResBotsCreate"]],
        [['text'=>$buttonValues['back'],'callback_data'=>"adminMenu"]],
    ]];
    editText($message_id, "🤖 مدیریت ربات ها", $keys);
}

// ===== Admin Backup Menu =====
if($data=='adminBackupMenu' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    // State format: {"enabled":0|1,"interval":2,"last":0}
    $st = getSettingValue('AUTO_BACKUP_STATE', '{"enabled":0,"interval":2,"last":0}');
    $stj = json_decode($st, true);
    if(!is_array($stj)) $stj = ["enabled"=>0,"interval"=>2,"last"=>0];
    $enabled = (int)($stj['enabled'] ?? 0);
    ...
    $keys = ['inline_keyboard'=>[
        [['text'=>'🗄 بکاپ دستی','callback_data'=>'adminBackupManual']],
        [['text'=>($enabled?"✅ بکاپ خودکار: روشن":"❌ بکاپ خودکار: خاموش") . " | هر {$interval} دقیقه",'callback_data'=>'adminBackupAuto']],
        [['text'=>'📤 افزودن/بازگردانی بکاپ','callback_data'=>'adminBackupRestore']],
        [['text'=>$buttonValues['back'],'callback_data'=>'adminMenu']],
    ]];
    editText($message_id, "🗄 مدیریت بکاپ

• بکاپ دستی: همین الان فایل SQL ارسال می‌شود
• بکاپ خودکار: به صورت دوره‌ای برای ادمین ارسال می‌شود
• افزودن/بازگردانی: فایل SQL را ارسال کنید", $keys);
}

// --- Manual backup now
if($data=='adminBackupManual' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    alert('در حال ساخت بکاپ...');
    $sql = dbCreateSqlBackup();
    if(!$sql){
        alert('❌ خطا در ساخت بکاپ');
        $data = 'adminBackupMenu';
    }else{
        $tmp = sys_get_temp_dir() . '/deltabot_backup_' . time() . '.sql';
        file_put_contents($tmp, $sql);
        $cap = "✅ بکاپ دیتابیس
" . date('Y-m-d H:i:s');
        sendDocument($tmp, $cap, ['inline_keyboard'=>[[['text'=>$buttonValues['back'],'callback_data'=>'adminBackupMenu']]]]);
        @unlink($tmp);
        exit;
    }
}

// --- Auto backup menu (toggle + interval)
if($data=='adminBackupAuto' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $st = getSettingValue('AUTO_BACKUP_STATE', '{"enabled":0,"interval":2,"last":0}');
    $stj = json_decode($st, true);
    if(!is_array($stj)) $stj = ["enabled"=>0,"interval"=>2,"last"=>0];
    $enabled = (int)($stj['enabled'] ?? 0);
    ...
    $keys = ['inline_keyboard'=>[
        [['text'=>($enabled?"✅ خاموش کن بکاپ خودکار":"✅ روشن کن بکاپ خودکار"),'callback_data'=>'adminBackupAutoToggle']],
        [['text'=>"⏱ تنظیم هر 2 دقیقه",'callback_data'=>'adminBackupSetInt_2'],['text'=>"هر 5 دقیقه",'callback_data'=>'adminBackupSetInt_5']],
        [['text'=>"هر 10 دقیقه",'callback_data'=>'adminBackupSetInt_10'],['text'=>"هر 30 دقیقه",'callback_data'=>'adminBackupSetInt_30']],
        [['text'=>"هر 60 دقیقه",'callback_data'=>'adminBackupSetInt_60'],['text'=>"سفارشی...",'callback_data'=>'adminBackupSetIntCustom']],
        [['text'=>$buttonValues['back'],'callback_data'=>'adminBackupMenu']],
    ]];
    editText($message_id, "⏱ بکاپ خودکار

وضعیت: ".($enabled?'روشن ✅':'خاموش ❌')."
بازه فعلی: هر {$interval} دقیقه

هر بار که زمانش برسد، فایل SQL برای ادمین ارسال می‌شود.", $keys);
}

if($data=='adminBackupAutoToggle' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $st = getSettingValue('AUTO_BACKUP_STATE', '{"enabled":0,"interval":2,"last":0}');
    $stj = json_decode($st, true);
    if(!is_array($stj)) $stj = ["enabled"=>0,"interval"=>2,"last"=>0];
    $stj['enabled'] = ((int)($stj['enabled'] ?? 0)) ? 0 : 1;
    upsertSettingValue('AUTO_BACKUP_STATE', json_encode($stj, JSON_UNESCAPED_UNICODE));
    alert($stj['enabled'] ? "✅ بکاپ خودکار روشن شد" : "❌ بکاپ خودکار خاموش شد");
    $data = 'adminBackupAuto';
}

if(preg_match('/^adminBackupSetInt_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $int = (int)$m[1];
    if($int < 1) $int = 1;
    $st = getSettingValue('AUTO_BACKUP_STATE', '{"enabled":0,"interval":2,"last":0}');
    $stj = json_decode($st, true);
    ...
    $stj['interval'] = $int;
    upsertSettingValue('AUTO_BACKUP_STATE', json_encode($stj, JSON_UNESCAPED_UNICODE));
    alert("✅ بازه بکاپ خودکار شد هر {$int} دقیقه");
    $data = 'adminBackupAuto';
}

if($data=='adminBackupSetIntCustom' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    setUser("adminBackupSetIntCustom","step");
    editText($message_id, "عدد دقیقه را ارسال کنید (مثلا 2):", ['inline_keyboard'=>[[['text'=>$buttonValues['cancel'],'callback_data'=>'adminBackupAuto']]]]);
}

if(isset($text) && $userInfo['step']=='adminBackupSetIntCustom' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    if(is_numeric($text)){
        $int = (int)$text;
        if($int < 1) $int = 1;
        $st = getSettingValue('AUTO_BACKUP_STATE', '{"enabled":0,"interval":2,"last":0}');
        ...
        upsertSettingValue('AUTO_BACKUP_STATE', json_encode($stj, JSON_UNESCAPED_UNICODE));
        setUser("none","step");
        sendMessage("✅ تنظیم شد: هر {$int} دقیقه", ['inline_keyboard'=>[[['text'=>$buttonValues['back'],'callback_data'=>'adminBackupAuto']]]]);
        exit;
    }else{
        sendMessage("فقط عدد بفرست 🙂", null);
        exit;
    }
}

// ===== Restore backup (SQL upload) =====
if($data=='adminBackupRestore' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    setUser("adminBackupRestore","step");
    editText($message_id, "📤 فایل بکاپ SQL را همینجا ارسال کنید.

⚠️ توجه: بازگردانی بکاپ باعث جایگزینی کامل دیتابیس فعلی می‌شود.", ['inline_keyboard'=>[[['text'=>$buttonValues['back'],'callback_data'=>'adminBackupMenu']]]]);
}
if(preg_match('/^adminResBotsList_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $per = 15;
    $off = $page*$per;
    $res = $connection->query("SELECT rb.*, u.name as uname FROM reseller_bots rb LEFT JOIN users u ON u.userid=rb.owner_userid WHERE rb.status=1 ORDER BY rb.id DESC LIMIT $per OFFSET $off");
    $rows = [];
    ...
    $rows[]=$nav;
    $rows[]=[['text'=>$buttonValues['back'],'callback_data'=>"adminResellerBots"]];
    editText($message_id, "📋 لیست ربات ها", ['inline_keyboard'=>$rows]);
}
...
if(preg_match('/^adminResBot_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    if(!$b){
        editText($message_id,"❌ پیدا نشد.",['inline_keyboard'=>[[['text'=>$buttonValues['back'],'callback_data'=>"adminResBotsList_0"]]]]);
    }else{
...
";
        $keys=['inline_keyboard'=>[
            ...
            [['text'=>$buttonValues['back'],'callback_data'=>"adminResBotsList_0"]],
        ]];
...
if(preg_match('/^adminResBotDelete_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    editText($message_id,"✅ حذف شد.",['inline_keyboard'=>[[['text'=>$buttonValues['back'],'callback_data'=>"adminResBotsList_0"]]]]);
}
...
if(preg_match('/^adminResBotRenew_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $rows[]=[['text'=>$buttonValues['back'],'callback_data'=>"adminResBot_" . $rid]];
    editText($message_id,"پلن تمدید را انتخاب کنید:",['inline_keyboard'=>$rows]);
}
...
if(preg_match('/^adminResBotDoRenew_(\d+)_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    if(!$b || !$p){
        ...
        editText($message_id,"✅ تمدید شد.
انقضا: ".jdate('Y/m/d H:i',$newExp),['inline_keyboard'=>[[['text'=>$buttonValues['back'],'callback_data'=>"adminResBot_" . $rid]]]]);
    }
}
...
if(preg_match('/^adminResBotTransfer_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    editText($message_id,"آیدی عددی کاربر جدید را ارسال کنید:",['inline_keyboard'=>[[['text'=>$buttonValues['cancel'],'callback_data'=>"adminResBot_" . $rid]]]]);
}

if($data=='adminResBotsCreate' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ensureResellerTables();
    setUser("adminResBotsCreateUser","step");
    editText($message_id,"آیدی عددی کاربر مالک را ارسال کنید:",['inline_keyboard'=>[[['text'=>$buttonValues['cancel'],'callback_data'=>"adminResellerBots"]]]]);
}
...
if(!$isChildBot && $data == "resellerShop"){
    ...
    if(count($plans) == 0){
        editText($message_id, "❌ پلنی برای ربات نمایندگی تعریف نشده است.
...
از ادمین بخواهید از پنل مدیریت اضافه کند.", ['inline_keyboard'=>[[['text'=>$buttonValues['back_to_main'],'callback_data'=>'mainMenu']]]]);
        ...
        editText($message_id, "🤖 ربات نمایندگی
...
if(!$isChildBot && preg_match('/^resPlan_(\d+)/',$data,$m)){
    ...
    if(!$plan){
        editText($message_id, "❌ پلن پیدا نشد.", ['inline_keyboard'=>[[['text'=>$buttonValues['back_to_main'],'callback_data'=>'mainMenu']]]]);
    }else{
        $about = "با سلام🩵
"
...
"
        ...
        editText($message_id, $about, $keys, "HTML");
    }
}
...
if(!$isChildBot && preg_match('/^resAgreePay_(\d+)/',$data,$m)){
    ...
    if(!$plan){
        ...
        if($wallet < $need){
            editText($message_id, "❌ موجودی کیف پول شما کافی نیست.
...
ابتدا کیف پول را شارژ کنید.", ['inline_keyboard'=>[[['text'=>$buttonValues['back_to_main'],'callback_data'=>'mainMenu']]]]);
            ...
            editText($message_id, "✅ پرداخت با موفقیت انجام شد.
...
if(!$isChildBot && preg_match('/^myResellerBots$/',$data)){
    ensureResellerTables();
    $stmt = $connection->prepare("SELECT * FROM `reseller_bots` WHERE `owner_userid`=? AND `status`=1 ORDER BY `id` DESC LIMIT 50");
    $stmt->bind_param("i",$from_id);
    $stmt->execute();
    $res = $stmt->get_result();
    $stmt->close();
    ...
    if($res && $res->num_rows>0){
        while($b = $res->fetch_assoc()){
            ...
            $rows[] = [['text'=> $uname . " | #" . $b['id'], 'callback_data'=>"myResBot_" . $b['id']]];
        }
    }else{
        $rows[] = [['text'=>"هیچ رباتی ندارید",'callback_data'=>"noop"]];
    }
    $rows[] = [['text'=>$buttonValues['back_to_main'],'callback_data'=>'mainMenu']];
    editText($message_id, "🤖 ربات های من", ['inline_keyboard'=>$rows]);
}

if(!$isChildBot && preg_match('/^myResBot_(\d+)/',$data,$m)){
    ensureResellerTables();
    $rid = (int)$m[1];
    $stmt = $connection->prepare("SELECT * FROM `reseller_bots` WHERE `id`=? AND `owner_userid`=? LIMIT 1");
    ...
    if(!$b){
        editText($message_id, "❌ ربات پیدا نشد.", ['inline_keyboard'=>[[['text'=>$buttonValues['back_to_main'],'callback_data'=>'mainMenu']]]]);
    }else{
        $uname = $b['bot_username'] ? '@'.$b['bot_username'] : '---';
...
";
        $keys = ['inline_keyboard'=>[
            ...
            [['text'=>$buttonValues['update_reseller_bot'],'callback_data'=>"resUpdate_" . $b['id']]],
...
if(!$isChildBot && preg_match('/^resDelete_(\d+)/',$data,$m)){
    ...
    if(!$b){
        ...
        $stmt = $connection->prepare("UPDATE `reseller_bots` SET `status`=0 WHERE `id`=?");
        $stmt->bind_param("i",$rid);
        $stmt->execute();
        $stmt->close();
        editText($message_id, "✅ ربات حذف شد.", ['inline_keyboard'=>[[['text'=>$buttonValues['back_to_main'],'callback_data'=>'myResellerBots']]]]);
    }
}
...
if(!$isChildBot && preg_match('/^resDoRenew_(\d+)_(\d+)/',$data,$m)){
    ...
    if(!$b || !$plan){
        editText($message_id, "❌ اطلاعات پیدا نشد.", ['inline_keyboard'=>[[['text'=>$buttonValues['back_to_main'],'callback_data'=>'mainMenu']]]]);
    }else{
        $need = (int)$plan['price'];
        $wallet = (int)($userInfo['wallet'] ?? 0);
        if($wallet < $need){
            editText($message_id, "❌ موجودی کافی نیست.
...
هزینه: ".number_format($need)." تومان", ['inline_keyboard'=>[[['text'=>$buttonValues['back_to_main'],'callback_data'=>"myResBot_" . $rid]]]]);
            ...
            editText($message_id, "✅ تمدید انجام شد.
...
if(preg_match('/^adminUsersList(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $off = (int)$m[1];
    editText($message_id, "👥 لیست کل کاربران", getAdminUsersListKeys($off), "HTML");
}
if(preg_match('/^adminDiscountUsers(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $off = (int)$m[1];
    editText($message_id, "٪ کاربران دارای تخفیف", getAdminDiscountUsersKeys($off), "HTML");
}
if(preg_match('/^adminUser_(\d+)_([0-9]+)$/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = $m[1];
    $backOff = (int)$m[2];
    $txt = getAdminUserDetailsText($uid);
    editText($message_id, $txt, getAdminUserDetailsKeys("adminUsersList{$backOff}"), "HTML");
}
if(preg_match('/^adminUser_(\d+)_disc([0-9]+)$/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = $m[1];
    $backOff = (int)$m[2];
    $txt = getAdminUserDetailsText($uid);
    editText($message_id, $txt, getAdminUserDetailsKeys("adminDiscountUsers{$backOff}"), "HTML");
}
...
function refreshUserInfoPanel($uid, $msgId=null){
    ...
    if($keys != null){
        editText($msgId, renderUserInfoTitle($uid), $keys, "HTML");
    }else{
...
if($data=="renameButtons" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    editText($message_id,"✏️ تغییر اسم دکمه‌ها",getRenameButtonsKeys(0));
}
if(preg_match('/^renameButtonsPage(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    editText($message_id,"✏️ تغییر اسم دکمه‌ها",getRenameButtonsKeys($match[1]));
}
...
if($data=="arrangeButtons" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    editText($message_id,"↕️ چینش دکمه‌ها",getArrangeButtonsMenuKeys());
}
if($data=="arrangeCustomButtons" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    editText($message_id,"↕️ چینش دکمه‌های سفارشی",getArrangeMainButtonsKeys());
}
...
if(preg_match('/^moveMainBtn_(up|down)_(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    editText($message_id,"↕️ چینش دکمه‌ها",getArrangeMainButtonsKeys());
}
...
if($data == "managePanels" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    editText($message_id, "🧩 مدیریت پنل‌ها", getPanelManagementKeys());
}
if($data == "generalSettings" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    editText($message_id, "⚙️ تنظیمات عمومی", getGeneralSettingsKeys());
}
