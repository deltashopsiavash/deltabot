...
// Auto DB backup (interval minutes) - sends to bot admin when they interact
if(($from_id == $admin || ($userInfo['isAdmin'] ?? false) == true)){
    $st = getSettingValue('AUTO_BACKUP_STATE', '{"enabled":0,"last":0,"interval_min":1440}');
    ...
    if($enabled && (time() - $last > $intervalSec)){
        // NOTE: must be non-blocking as much as possible on shared hosts
        // Use file-based dump (mysqldump when available, otherwise streaming dump)
        $tmp = dbCreateSqlBackupFile('deltabot_auto_backup');
        if($tmp){
            if(@filesize($tmp) > 49*1024*1024){
                // too large, skip sending (but do not stop bot from responding)
                $stj['last'] = time();
        ...
        if($fileId){
            ...
            if($in && $out){
                ...
                if($tokenToUse && isShellExecAvailable()){
                    ...
                    $cmd = 'nohup php ' . escapeshellarg($worker) . ' restore ' . escapeshellarg($tokenToUse) . ' ' . escapeshellarg($from_id) . ' ' . escapeshellarg($path) . ' >/dev/null 2>&1 &';
                    @shell_exec($cmd);
                }else{
...
⚠️توجه کنید⚠️ ایدی عددی باید فقط عدد باشه و هیج چیز اضافه دیگه ای نباشه");
    ...
    if(preg_match('/^resellerAwaitAdmin_(\d+)$/', $userInfo['step'] ?? '', $mm)){
        ...
        $rowRes = $connection->query("SELECT `bot_token`,`expires_at`,`bot_username`,`admin_userid` FROM `reseller_bots` WHERE `id`={$rid} LIMIT 1");
        $row = $rowRes ? $rowRes->fetch_assoc() : null;
        if(!is_array($row)){
            $row = ['bot_token'=>'','expires_at'=>0,'bot_username'=>null,'admin_userid'=>$aid];
        }
        if(!empty($row['bot_token'])){
            // Even if setWebhook fails for any reason, we still continue and show 100%.
...
if($data=='adminBackupGet' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    if(isShellExecAvailable()){
        $worker = __DIR__ . '/backup_worker.php';
        $cmd = 'nohup php ' . escapeshellarg($worker) . ' backup ' . escapeshellarg($tokenToUse) . ' ' . escapeshellarg($from_id) . ' ' . escapeshellarg('deltabot_backup') . ' >/dev/null 2>&1 &';
        @shell_exec($cmd);
    }else{
        // Fallback (sync) - may take time on large DB
...
if(!$isChildBot && preg_match('/^myResellerBots$/',$data)){
    ...
    $stmt = $connection->prepare("SELECT * FROM `reseller_bots` WHERE `owner_userid`=? ORDER BY `id` DESC LIMIT 50");
    $stmt->bind_param("i",$from_id);
    $stmt->execute();
    $res = $stmt->get_result();
    $stmt->close();
