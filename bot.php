...
if(($data=='adminResellerBots' || $text=='🤖 ربات‌ها' || $text=='🤖 ربات ها' || $text=='مدیریت ربات ها 🤖' || $text=='🤖 مدیریت ربات ها') && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ensureResellerTables();
    ...
    smartSendOrEdit("🤖 مدیریت ربات ها", $keys);
}

// ===== Admin Backup Menu =====
if(($data=='adminBackupMenu' || $text=='🗄 بکاپ' || $text=='مدیریت بکاپ 💾' || $text=='🗄 مدیریت بکاپ' || $text=='مدیریت بکاپ') && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $st = getSettingValue('AUTO_BACKUP_STATE', '{"enabled":0,"last":0}');
    $stj = json_decode($st, true);
    $enabled = (int)($stj['enabled'] ?? 0);
    $keys = ['inline_keyboard'=>[
