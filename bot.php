...
|,/', $raw);
            ...
            if(count($parts) > 0) $paymentKeys['bankAccount'] = $parts;
        }
    }
...
// User actions from user reports panel
if(preg_match('/^userAct_(\w+)_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $action = $m[1];
    $uid = (int)$m[2];

    if($action == "inc"){
        editText($message_id,"مبلغ افزایش را وارد کنید:",$cancelKey);
        setUser("increaseWalletUser" . $uid);
        exit;
    }
    if($action == "dec"){
        editText($message_id,"مبلغ کاهش را وارد کنید:",$cancelKey);
        setUser("decreaseWalletUser" . $uid);
        exit;
    }
    if($action == "pm"){
        editText($message_id,'🔘|لطفا پیامت رو بفرست',$cancelKey);
        setUser("sendMessageToUser" . $uid);
        exit;
    }
    if($action == "ban"){
        ...
        editText($message_id,"اطلاعات کاربر",getUserInfoKeys($uid),"html");
        exit;
    }
    if($action == "unban"){
    ...
    if($action == "discount"){
        editText($message_id,"درصد تخفیف کاربر را وارد کنید (مثلا 10)\nبرای حذف: /off",$cancelKey);
        setUser("setUserDiscount_" . $uid);
        exit;
    }
    if($action == "test"){
        $stmt = $connection->prepare("SELECT `freetrial` FROM `users` WHERE `userid` = ?");
        $stmt->bind_param("i",$uid);
        $stmt->execute();
        $freetrial = $stmt->get_result()->fetch_assoc()['freetrial'] ?? null;
        $stmt->close();
        $isBlocked = (!is_null($freetrial) && $freetrial == "used");
        $txt = $isBlocked ? "اکانت تست این کاربر در حال حاضر محدود است." : "اکانت تست این کاربر آزاد است.";
        $keys = json_encode(['inline_keyboard'=>[
            [
                ['text'=>$isBlocked?"✅ آزاد کن":"🧪 محدود کن",'callback_data'=>"toggleUserTest_" . $uid]
            ],
            [['text'=>$buttonValues['back_button'],'callback_data'=>"userReportsBack_" . $uid]]
        ]]);
    ...
    if($action == "orders"){
        $offset = 0;
        $stmt = $connection->prepare("SELECT * FROM `orders_list` WHERE `userid` = ? ORDER BY `id` DESC LIMIT 10 OFFSET ?");
        $stmt->bind_param("si",$uid,$offset);
        $stmt->execute();
        $list = $stmt->get_result();
        $stmt->close();
        $txt = "📜 لیست سفارش های کاربر $uid\n\n";
        if($list->num_rows == 0){
            $txt .= "سفارشی یافت نشد.";
        }else{
            while($row = $list->fetch_assoc()){
                $txt .= "• #".$row['id']." | ".number_format($row['amount'])." تومان | ".$row['date']."\n".$row['remark']."\n\n";
            }
        }
        $keys = json_encode(['inline_keyboard'=>[
            [['text'=>$buttonValues['back_button'],'callback_data'=>"userReportsBack_" . $uid]]
    ...
    if($action == "exauto"){
        ...
        $stmt->bind_param("i",$uid);
        $stmt->execute();
        $temp = $stmt->get_result()->fetch_assoc()['temp'] ?? null;
        $stmt->close();
        $tempArr = json_decode($temp,true);
        if(!is_array($tempArr)) $tempArr = array();
        $cur = isset($tempArr['exclude_cart_auto']) && $tempArr['exclude_cart_auto'] == 1;
        $tempArr['exclude_cart_auto'] = $cur?0:1;
        $newTemp = json_encode($tempArr);
        $stmt = $connection->prepare("UPDATE `users` SET `temp` = ? WHERE `userid` = ?");
        $stmt->bind_param("si",$newTemp,$uid);
        $stmt->execute();
        $stmt->close();
        alert($cur?"❎ استثنا برداشته شد":"🛑 کاربر از تایید خودکار مستثنا شد");
        editText($message_id,"اطلاعات کاربر",getUserInfoKeys($uid),"html");
        exit;
    }
}
// Back to selected user panel
if(preg_match('/^userReportsBack_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = (int)$m[1];
    editText($message_id,"اطلاعات کاربر",getUserInfoKeys($uid),"html");
    exit;
}
if(preg_match('/^toggleUserTest_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = (int)$m[1];
    $stmt = $connection->prepare("SELECT `freetrial` FROM `users` WHERE `userid` = ?");
    $stmt->bind_param("i",$uid);
    $stmt->execute();
    ...
    $isBlocked = (!is_null($freetrial) && $freetrial == "used");
    if($isBlocked){
        $stmt = $connection->prepare("UPDATE `users` SET `freetrial` = NULL WHERE `userid` = ?");
    }else{
        $stmt = $connection->prepare("UPDATE `users` SET `freetrial` = 'used' WHERE `userid` = ?");
    }
    $stmt->bind_param("i",$uid);
    $stmt->execute();
    $stmt->close();
    alert($isBlocked?"✅ آزاد شد":"🧪 محدود شد");
    // reopen menu
    $data = "userAct_test_" . $uid;

if(preg_match('/^setUserDiscount_(\d+)/',$userInfo['step'],$m) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    $uid = (int)$m[1];
    if(trim($text) == "/off"){
        $stmt = $connection->prepare("UPDATE `users` SET `discount_percent` = NULL WHERE `userid` = ?");
        $stmt->bind_param("i",$uid);
        $stmt->execute();
        $stmt->close();
        sendMessage("✅ تخفیف کاربر حذف شد",$removeKeyboard);
        sendMessage("اطلاعات کاربر",getUserInfoKeys($uid),"html");
        setUser();
        exit;
    }
    if(!is_numeric($text) || $text < 0 || $text > 100){
        sendMessage("لطفا یک عدد بین 0 تا 100 بفرستید");
        exit;
    }
    $new = json_encode(['normal'=>(string)$text]);
    $stmt = $connection->prepare("UPDATE `users` SET `discount_percent` = ? WHERE `userid` = ?");
    $stmt->bind_param("si",$new,$uid);
    $stmt->execute();
    $stmt->close();
    sendMessage("✅ تخفیف کاربر تنظیم شد",$removeKeyboard);
    sendMessage("اطلاعات کاربر",getUserInfoKeys($uid),"html");
    setUser();
    exit;
}
...
if(preg_match('/increaseWalletWithCartToCart(?<hashId>.*)/',$data, $match)) {
    ...
    sendMessage(str_replace(["ACCOUNT-NUMBER", "HOLDER-NAME"],[getRandomBankAccount($paymentKeys),$paymentKeys['holderName']], $mainValues['increase_wallet_cart_to_cart']),$cancelKey, "HTML");
    exit;
}
...
if(preg_match('/^yesSend2All(\d+)/', $data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    editText($message_id,"قبل ارسال، پیام در پی وی کاربران پین بشه؟",json_encode(['inline_keyboard'=>[
        [['text'=>"✅ بله پین کن",'callback_data'=>"yesPin2All" . $match[1]],['text'=>"❌ نه",'callback_data'=>"noPin2All" . $match[1]]]
    ]]));
}
if(preg_match('/^yesPin2All(\d+)/', $data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $stmt = $connection->prepare("UPDATE `send_list` SET `state` = 1, `pin` = 1 WHERE `id` = ?") ;
    $stmt->bind_param('i', $match[1]);
    $stmt->execute();
    $stmt->close();
...
if(preg_match('/^noPin2All(\d+)/', $data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $stmt = $connection->prepare("UPDATE `send_list` SET `state` = 1, `pin` = 0 WHERE `id` = ?") ;
    $stmt->bind_param('i', $match[1]);
    $stmt->execute();
    $stmt->close();
...
if(preg_match('/payCustomWithCartToCart(.*)/',$data, $match)) {
    ...
    setUser($data);
    delMessage();
    sendMessage(str_replace(["ACCOUNT-NUMBER", "HOLDER-NAME"],[getRandomBankAccount($paymentKeys),$paymentKeys['holderName']], $mainValues['buy_account_cart_to_cart']),$cancelKey, "HTML");
    exit;
}
...
if(preg_match('/^sendTicket_(.*)/',$userInfo['step'],$match)  and $text!=$buttonValues['cancel']){
    if(isset($text) || isset($update->message->photo)){
        ...
        if(isset($text)){
            ...
            $stmt->bind_param("iis", $chatRowId, $time, $text);
            sendPhotoToAdmins($fileid, $txt,$keys, "HTML");
        }
        $stmt->execute();
        $stmt->close();
...
if($data=="renameMainButtons" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    editText($message_id,"تغییر اسم دکمه ها",getRenameMainButtonsKeys());
}
if($data=="orderMainButtons" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    editText($message_id,"چینش دکمه ها",getOrderMainButtonsKeys());
}
if($data=="renameStartButton" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    delMessage();
    sendMessage("متن جدید دکمه استارت را ارسال کنید",$cancelKey);
    setUser("renameStartButton");
}
if($userInfo['step']=="renameStartButton" && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    if(trim($text) == "/empty") $text = "";
    $stmt = $connection->prepare("SELECT * FROM `setting` WHERE `type` = 'START_BUTTON_TEXT'");
    $stmt->execute();
    $exists = $stmt->get_result();
    $stmt->close();
    if($exists->num_rows > 0) $stmt = $connection->prepare("UPDATE `setting` SET `value` = ? WHERE `type` = 'START_BUTTON_TEXT'");
    else $stmt = $connection->prepare("INSERT INTO `setting` (`type`, `value`) VALUES ('START_BUTTON_TEXT', ?)");
    $stmt->bind_param("s",$text);
    $stmt->execute();
    $stmt->close();
    sendMessage($mainValues['saved_successfuly'],$removeKeyboard);
...
if(preg_match('/^renameMainButton(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    delMessage();
    sendMessage("اسم جدید دکمه را وارد کنید",$cancelKey);
    setUser("renameMainButton_" . $match[1]);
}
if(preg_match('/^renameMainButton_(\d+)/',$userInfo['step'],$match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    $newTitle = trim($text);
    if(empty($newTitle)){
        sendMessage("اسم دکمه نمی تواند خالی باشد");
        exit();
    }
    ...
    $stmt->close();
    if(!$row){
        sendMessage("دکمه یافت نشد");
        setUser();
        exit();
    }
    $newType = "MAIN_BUTTONS" . $newTitle;
    $stmt = $connection->prepare("UPDATE `setting` SET `type` = ? WHERE `id` = ?");
    $stmt->bind_param("si",$newType,$match[1]);
    $stmt->execute();
    $stmt->close();
    sendMessage($mainValues['saved_successfuly'],$removeKeyboard);
    sendMessage("تغییر اسم دکمه ها",getRenameMainButtonsKeys());
    setUser();
}
if(preg_match('/^moveMainButtonUp(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $stmt = $connection->prepare("SELECT * FROM `setting` WHERE `type` LIKE '%MAIN_BUTTONS%' ORDER BY `id` ASC");
    $stmt->execute();
    $rows = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    $stmt->close();
    for($i=0;$i<count($rows);$i++){
        if($rows[$i]['id'] == $match[1] && $i>0){
            $a = $rows[$i];
            $b = $rows[$i-1];
            $stmt = $connection->prepare("UPDATE `setting` SET `type` = ?, `value` = ? WHERE `id` = ?");
            $stmt->bind_param("ssi",$a['type'],$a['value'],$b['id']);
            $stmt->execute();
            $stmt->close();
            $stmt = $connection->prepare("UPDATE `setting` SET `type` = ?, `value` = ? WHERE `id` = ?");
            $stmt->bind_param("ssi",$b['type'],$b['value'],$a['id']);
            $stmt->execute();
            $stmt->close();
            break;
        }
...
if(preg_match('/^moveMainButtonDown(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $stmt = $connection->prepare("SELECT * FROM `setting` WHERE `type` LIKE '%MAIN_BUTTONS%' ORDER BY `id` ASC");
    $stmt->execute();
    $rows = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    $stmt->close();
    for($i=0;$i<count($rows);$i++){
        if($rows[$i]['id'] == $match[1] && $i < count($rows)-1){
            $a = $rows[$i];
            $b = $rows[$i+1];
            $stmt = $connection->prepare("UPDATE `setting` SET `type` = ?, `value` = ? WHERE `id` = ?");
            $stmt->bind_param("ssi",$a['type'],$a['value'],$b['id']);
...
if(preg_match('/payRenewWithCartToCart(.*)/',$data,$match)) {
    ...
    sendMessage(str_replace(['ACCOUNT-NUMBER', 'HOLDER-NAME'],[getRandomBankAccount($paymentKeys), $paymentKeys['holderName']], $mainValues['renew_ccount_cart_to_cart']),$cancelKey,"html");
    exit;
}
