...
if(($userInfo['step'] ?? '') == 'awaiting_main_buttons_order' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    // Build current ordered list (ids)
    ...
    if($nTotal == 0){
        setUser('none','step');
        sendMessage("❌ دکمه سفارشی‌ای پیدا نشد.");
        exit;
    }
    // Validate numbers
    $seen=[];
    ...
    $newOrderIds=[];
    foreach($nums as $n){
        $newOrderIds[] = (int)$ordered[$n-1]['id'];
    }
    upsertSettingValue('MAIN_BUTTONS_ORDER', json_encode($newOrderIds, JSON_UNESCAPED_UNICODE));

    setUser('none','step');
    sendMessage("✅ چینش ذخیره شد.");
    $data = 'arrangeCustomButtons';
}
...
if($data=="arrangeMainOrderText" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    if(count($ordered)==0){
        smartSendOrEdit($message_id, "❌ دکمه سفارشی‌ای پیدا نشد.", ['inline_keyboard'=>[[['text'=>$buttonValues['back_button'],'callback_data'=>'arrangeCustomButtons']]]]);
        exit;
    }
    $lines = [];
    ...
    $msg = "✏️ ترتیب جدید دکمه‌ها را با ارسال شماره‌ها مشخص کنید.\n\n".
           implode("\n",$lines)."\n\n".
           "مثال: 3,1,2\n".
           "یا اگر می‌خواهید همین ترتیب بماند: ".implode(',', range(1,count($ordered)));
    smartSendOrEdit($message_id, $msg, ['inline_keyboard'=>[[['text'=>$buttonValues['back_button'],'callback_data'=>'arrangeCustomButtons']]]]);
    exit;
}
