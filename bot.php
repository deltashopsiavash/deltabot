...
لطفا دوباره توکن صحیح رو ارسال کن.");
        ...
        $fromIdStr = (string)$from_id;
        $botIdStr = (string)$botId;
        $stmt->bind_param("sssis", $token, $botIdStr, $username, $rid, $fromIdStr);
        $stmt->execute();
        $stmt->close();
...
⚠️توجه کنید⚠️ ایدی عددی باید فقط عدد باشه و هیج چیز اضافه دیگه ای نباشه");
    ...
    if(preg_match('/^resellerAwaitAdmin_(\d+)$/', $userInfo['step'] ?? '', $mm)){
        ...
        $aidStr = (string)$adminId;
        $fromIdStr = (string)$from_id;
        $stmt->bind_param("sis", $aidStr, $rid, $fromIdStr);
        $stmt->execute();
        $stmt->close();
...
if(!$isChildBot && (($data ?? "") == "resellerShop" || (isset($text) && $text == ($buttonValues['reseller_bot_shop'] ?? '')))){
    ensureResellerTables();
    // list active plans
    $plans = [];
    $res = $connection->query("SELECT * FROM `reseller_plans` WHERE `is_active`=1 ORDER BY `id` ASC");
...
ابتدا کیف پول را شارژ کنید.", ['inline_keyboard'=>[[['text'=>$buttonValues['back_to_main'],'callback_data'=>'mainMenu']]]]);
            ...
            $zero = '0';
            $fromIdStr = (string)$from_id;
            $stmt->bind_param("sssii", $fromIdStr, $emptyToken, $zero, $now, $expires);
$stmt->execute();
            $rid = $stmt->insert_id;
            $stmt->close();
...
if(!$isChildBot && (((string)($data ?? '')) === 'myResellerBots' || (isset($text) && $text == ($buttonValues['my_reseller_bots'] ?? '')))){
    ensureResellerTables();
    $stmt = $connection->prepare("SELECT * FROM `reseller_bots` WHERE `owner_userid`=? AND `status`=1 ORDER BY `id` DESC LIMIT 50");
    $fromIdStr = (string)$from_id;
    $stmt->bind_param("s",$fromIdStr);
    $stmt->execute();
    $res = $stmt->get_result();
    $stmt->close();
...
if(!$isChildBot && preg_match('/^myResBot_(\d+)/',$data,$m)){
    ...
    $fromIdStr = (string)$from_id;
    $stmt->bind_param("is",$rid,$fromIdStr);
    $stmt->execute();
