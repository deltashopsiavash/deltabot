...
"
        ...
        // Set webhook after confirming to user, with curl timeouts to avoid hanging the main request.
...
if(preg_match('/^adminResDBList_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    smartSendOrEdit($message_id, $text, $keys, true);
}
...
if(preg_match('/^adminResDBDropYes_([A-Za-z0-9_]+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    if($ok){
        ...
        smartSendOrEdit($message_id, "✅ دیتابیس حذف شد: `{$dbn}`", ['inline_keyboard'=>[[['text'=>"🔙 برگشت به لیست",'callback_data'=>"adminResDBList_0"]]]], true);
    }else{
        $err = $connection->error;
        smartSendOrEdit($message_id, "❌ حذف انجام نشد.\n\nخطا: {$err}", ['inline_keyboard'=>[[['text'=>"🔙 برگشت",'callback_data'=>"adminResDBInfo_".$dbn]]]], true);
    }
    exit;
}
