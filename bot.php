...
// ===== Auto DB backup (main bot) =====
if(!$isChildBot && ($from_id == $admin || ($userInfo['isAdmin'] ?? false) == true)){
    ...
    if($enabled && $interval >= 1){
        ...
        if($last == 0 || ($now - $last) >= ($interval * 60)){
            $sql = dbCreateSqlBackup();
            if($sql){
                $tmp = sys_get_temp_dir() . '/deltabot_autobackup_' . date('Ymd_His') . '.sql';
                file_put_contents($tmp, $sql);
                sendDocument($tmp, "⏱ بکاپ خودکار دیتابیس\n".date('Y-m-d H:i:s'));
                @unlink($tmp);
                $stj['last'] = $now;
...
// ===== Set auto-backup interval (admin) =====
if(($userInfo['step'] ?? '') == 'awaiting_backup_interval' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $val = isset($text) ? (int)trim($text) : 0;
    if($val < 1){
        sendMessage("❌ مقدار نامعتبر است. فقط عدد دقیقه (مثلا 2) ارسال کنید.");
    }else{
        $st = getSettingValue('AUTO_BACKUP_STATE', '{"enabled":0,"interval":1440,"last":0}');
        $stj = json_decode($st, true);
        $stj['interval'] = $val;
        // reset next run by keeping last as-is
        upsertSettingValue('AUTO_BACKUP_STATE', json_encode($stj, JSON_UNESCAPED_UNICODE));
        setUser('none','step');
        sendMessage("✅ فاصله بکاپ خودکار تنظیم شد روی {$val} دقیقه.");
    }
    exit;
}
...
if($data=='adminBackupMenu' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keys = ['inline_keyboard'=>[
        ...
        [['text'=>($enabled ? "⏱ بکاپ خودکار: روشن (هر {$interval} دقیقه)" : "⏱ بکاپ خودکار: خاموش"),'callback_data'=>'adminBackupAutoMenu']],
...
- بکاپ خودکار: ارسال دوره‌ای (براساس دقیقه)
...
if($data=='adminBackupAutoMenu' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keys = ['inline_keyboard'=>[
        ...
        [['text'=>"✏️ تنظیم فاصله (دقیقه): {$interval}",'callback_data'=>'adminBackupSetInterval']],
...
برای تغییر فاصله روی «تنظیم فاصله» بزنید و فقط عدد ارسال کنید.", $keys);
}
...
if($data=='adminBackupSetInterval' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    editText($message_id, "✏️ فاصله بکاپ خودکار را به دقیقه ارسال کنید.
مثال: 2

⚠️ فقط عدد بفرستید.", ['inline_keyboard'=>[[['text'=>$buttonValues['back'],'callback_data'=>'adminBackupAutoMenu']]]]);
}
