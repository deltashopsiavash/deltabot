...
if(preg_match('/increaseWalletWithCartToCart(?<hashId>.*)/',$data, $match)) {
    ...
    sendMessage(renderCartToCartTemplate($mainValues['cart_to_cart_message'] ?? $mainValues['increase_wallet_cart_to_cart']), $cancelKey, "HTML");
    exit;
}
...
if(preg_match('/payCustomWithCartToCart(.*)/',$data, $match)) {
    ...
    setUser($data);
    delMessage();
    sendMessage(renderCartToCartTemplate($mainValues['cart_to_cart_message'] ?? $mainValues['buy_account_cart_to_cart']), $cancelKey, "HTML");
    exit;
}
...
if(preg_match('/^renameBtnKey_(.+)/',$userInfo['step'],$match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    ...
    $key = $match[1];
    // Some keys are message templates (MAINVALUE_*), not button labels
    $mainValueKeys = [
        "start_message",
    ...
    if(in_array($key, $mainValueKeys, true)){
        $type = "MAINVALUE_" . $key;
    }else{
