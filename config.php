...
function pickRandomValue($raw){
    $vals = parseMultiValues($raw);
    if(count($vals) == 0) return $raw;
    return $vals[array_rand($vals)];
}
function getAllAdmins(){
    global $connection, $admin;
    $admins = [];
    if(!empty($admin)) $admins[] = (int)$admin;
    ...
    while($row = $res->fetch_assoc()){
        $uid = (int)$row['userid'];
        if(!in_array($uid, $admins)) $admins[] = $uid;
    }
    return $admins;
}
function getUserManageKeys($userId){
    global $buttonValues;
    return json_encode(['inline_keyboard'=>[
        [['text'=>"➕ افزایش موجودی",'callback_data'=>"userAct_inc_$userId"],['text'=>"➖ کاهش موجودی",'callback_data'=>"userAct_dec_$userId"]],
        [['text'=>"🔒 مسدود کردن",'callback_data'=>"userAct_ban_$userId"],['text'=>"🔓 آزاد کردن",'callback_data'=>"userAct_unban_$userId"]],
        [['text'=>"✉️ پیام خصوصی",'callback_data'=>"userAct_pm_$userId"]],
        [['text'=>"🎯 درصد تخفیف",'callback_data'=>"userAct_disc_$userId"],['text'=>"🧪 محدودیت تست",'callback_data'=>"userAct_test_$userId"]],
        [['text'=>"🧾 لیست سفارش‌ها",'callback_data'=>"userAct_orders_$userId"]],
        [['text'=>"🚫 استثنا از تایید خودکار",'callback_data'=>"userAct_exempt_$userId"]],
        [['text'=>"🧹 صفرکردن موجودی",'callback_data'=>"userAct_reset_$userId"]],
        [['text'=>$buttonValues['back_button'],'callback_data'=>"managePanel"]],
    ]]);
}
...
[
    ['text'=>"➕ افزایش موجودی",'callback_data'=>"userAct_inc" . $userId . ""],
    ['text'=>"➖ کاهش موجودی",'callback_data'=>"userAct_dec" . $userId . ""]
],
[
    ['text'=>"🔒 مسدود کردن",'callback_data'=>"userAct_ban" . $userId . ""],
    ['text'=>"🔓 آزاد کردن",'callback_data'=>"userAct_unban" . $userId . ""]
],
[
    ['text'=>"✉️ پیام خصوصی",'callback_data'=>"userAct_pm" . $userId . ""],
    ['text'=>"🧾 سفارش‌ها",'callback_data'=>"userAct_orders" . $userId . ""]
],
[
    ['text'=>"🎯 تخفیف",'callback_data'=>"userAct_disc" . $userId . ""],
    ['text'=>"🧪 محدودیت تست",'callback_data'=>"userAct_test" . $userId . ""]
],
[
    ['text'=>"🚫 استثنا تایید خودکار",'callback_data'=>"userAct_exempt" . $userId . ""],
    ['text'=>"🧹 صفرکردن موجودی",'callback_data'=>"userAct_reset" . $userId . ""]
],
[
