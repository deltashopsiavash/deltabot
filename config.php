...
function botWithToken($token, $method, $datas = []){
    ...
    // Encode nested arrays/objects (e.g., reply_markup) for Telegram API
    foreach($datas as $k=>$v){
        if($v instanceof CURLFile) continue;
        if(is_array($v) || (is_object($v) && !($v instanceof CURLFile))){
            $datas[$k] = json_encode($v, JSON_UNESCAPED_UNICODE);
        }
    }
...
function ensureResellerTables(){
    ...
    $addColumnIfMissing('reseller_bots','backup_interval_minutes','`backup_interval_minutes` INT NOT NULL DEFAULT 1440');
    $addColumnIfMissing('reseller_bots','next_backup_at','`next_backup_at` INT NOT NULL DEFAULT 0');
}
...
if(!$isChildBot && (($botState['resellerBotState'] ?? 'on') == 'on')){
    if((($botState['resellerBotShopState'] ?? 'on') == 'on')){
        $mainKeys = array_merge($mainKeys,[[['text'=>$buttonValues['reseller_bot_shop'],'callback_data'=>"resellerShop"]]]);
    }
    $mainKeys = array_merge($mainKeys,[[['text'=>$buttonValues['my_reseller_bots'],'callback_data'=>"myResellerBots"]]]);
}
...
function getBotSettingKeys(){
    ...
    $resellerBotShop = ($botState['resellerBotShopState']??"on")=="on"?$buttonValues['on']:$buttonValues['off'];
