...
function botWithToken($token, $method, $datas = []){
    ...
    // Normalize reply_markup (Telegram expects JSON string)
    if(isset($datas['reply_markup']) && is_array($datas['reply_markup'])){
        $datas['reply_markup'] = json_encode($datas['reply_markup'], JSON_UNESCAPED_UNICODE);
    }
...
function bot($method, $datas = []){
    ...
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); 
    // Drop empty parse_mode to avoid Telegram errors
    if(isset($datas['parse_mode']) && (is_null($datas['parse_mode']) || trim((string)$datas['parse_mode']) === '')){
...
// Send database backup to all admins based on AUTO_BACKUP_STATE setting.
// AUTO_BACKUP_STATE JSON: {"enabled":0|1,"interval":minutes,"last":unix_ts}
function autoBackupTick(){
    global $isChildBot;
    if($isChildBot) return; // prevent duplicate backups from child instances
    ...
    if(!$enabled) return;
    $now = time();
    if($last > 0 && ($now - $last) < ($interval * 60)) return;
...
if(!$isChildBot && (($botState['resellerBotState'] ?? 'on') == 'on')){
    ...
    // Update button (only inside reseller/child bot and only for that bot admin)
    if($isChildBot && $from_id == $admin){
        $mainKeys = array_merge($mainKeys, [
            [['text'=>$buttonValues['update_reseller_bot'],'callback_data'=>'resellerBotUpdate']]
        ]);
    }
...
function getBotSettingKeys(){
    ...
    $resellerBotState = (($botState['resellerBotState'] ?? 'on')=='on')?$buttonValues['on']:$buttonValues['off'];
    ...
    return json_encode(['inline_keyboard'=>[
        ...
        [
            ['text'=> $resellerBotState,'callback_data'=>"changeBotresellerBotState"],
            ['text'=>"دکمه ربات نمایندگی",'callback_data'=>"deltach"]
            ],
        [
