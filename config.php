...
// Base URL of this bot installation (used for payment links + reseller bot webhooks).
// You can set it via env DELTABOT_BASE_URL, otherwise it will be auto-detected from the current request.
$botUrl = getenv('DELTABOT_BASE_URL') ?: '';
if(empty($botUrl)){
    $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $host = $_SERVER['HTTP_HOST'] ?? '';
    // dirname($_SERVER['SCRIPT_NAME']) usually gives /deltabot or similar
    $basePath = rtrim(str_replace('\\','/', dirname($_SERVER['SCRIPT_NAME'] ?? '')), '/');
    if($host){
        $botUrl = $scheme . '://' . $host . ($basePath ? $basePath . '/' : '/');
    }
}
// Ensure trailing slash
if(!empty($botUrl) && substr($botUrl, -1) !== '/') $botUrl .= '/';
