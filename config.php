...
function getAdminDiscountUsersKeys($offset=0){
    ...
    // Fetch candidates (non-empty discount_percent)
    $stmt = $connection->prepare("SELECT `userid`,`name`,`discount_percent` FROM `users` WHERE `discount_percent` IS NOT NULL AND `discount_percent` != '' ORDER BY `id` DESC");
    $stmt->execute();
    $res = $stmt->get_result();
    $stmt->close();
    ...
    $all = [];
    while($u = $res->fetch_assoc()){
        $dp = json_decode($u['discount_percent'], true);
        if(!is_array($dp)) continue;
        $normal = (int)($dp['normal'] ?? 0);
        $hasAny = $normal > 0;
        if(!$hasAny){
            // if any server/plan percent exists >0 consider it
            if(isset($dp['servers']) && is_array($dp['servers'])){
                foreach($dp['servers'] as $v){ if((int)$v>0){ $hasAny=true; break; } }
            }
            if(!$hasAny && isset($dp['plans']) && is_array($dp['plans'])){
                foreach($dp['plans'] as $v){ if((int)$v>0){ $hasAny=true; break; } }
            }
        }
        if(!$hasAny) continue;
        $u['__normal'] = $normal;
    ...
    foreach($slice as $u){
        ...
        $name = trim((string)$u['name']);
        if($name === '') $name = 'بدون نام';
        $normal = (int)($u['__normal'] ?? 0);
        $rows[] = [
            ['text'=>"👤 {$name} ({$uid}) - {$normal}%",'callback_data'=>"adminUser_{$uid}_disc{$offset}"],
        ];
    }
...
function getAdminUserDetailsText($userId){
    ...
    $dp = json_decode($u['discount_percent'] ?? '', true);
    $normal = is_array($dp) ? (int)($dp['normal'] ?? 0) : 0;
