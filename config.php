...
function getRandomBankAccount($paymentKeys){
    if(!isset($paymentKeys['bankAccount'])) return '';
    $acc = $paymentKeys['bankAccount'];
    if(is_array($acc)){
        $acc = array_values(array_filter($acc, function($v){ return !empty($v); }));
        if(count($acc) == 0) return '';
        return $acc[array_rand($acc)];
    }
    return $acc;
}

function sendMessageToAdmins($txt, $key = null, $parse ="MarkDown"){
    $ids = getAllAdminIds();
    foreach($ids as $aid){
        sendMessage($txt, $key, $parse, $aid);
    }
}
...
// Override start button text (optional)
$stmt = $connection->prepare("SELECT * FROM `setting` WHERE `type` = 'START_BUTTON_TEXT'");
$stmt->execute();
$startBtnText = $stmt->get_result()->fetch_assoc()['value'] ?? null;
$stmt->close();
if(!empty($startBtnText)) $buttonValues['start_bot'] = $startBtnText;

// Default referral button state if not set
if(!isset($botState['referralButtonState'])) $botState['referralButtonState'] = "on";
...
function getMainKeys(){
    ...
    $mainKeys = array_merge($mainKeys,[
        ...
        ((($botState['referralButtonState']??"on") == "on")?[['text'=>$buttonValues['invite_friends'],'callback_data'=>"inviteFriends"],['text'=>$buttonValues['my_info'],'callback_data'=>"myInfo"]]:[['text'=>$buttonValues['my_info'],'callback_data'=>"myInfo"]]),
...
function getAdminKeys(){
    ...
    return json_encode(['inline_keyboard'=>[
        ...
        ($from_id == $admin?[['text'=>$buttonValues['admins_list'],'callback_data'=>"adminsList"]]:[]),        [['text'=>$buttonValues['create_account'],'callback_data'=>"createMultipleAccounts"],
        ['text'=>$buttonValues['gift_volume_day'],'callback_data'=>"giftVolumeAndDay"]],        [['text'=>$buttonValues['search_admin_config'],'callback_data'=>"searchUsersConfig"]],
        ...
        [['text'=>$buttonValues['gateways_settings'],'callback_data'=>"gateWays_Channels"],['text'=>$buttonValues['bot_settings'],'callback_data'=>'botSettings']],        [['text'=>$buttonValues['forward_to_all'],'callback_data'=>"forwardToAll"]],
        [
...
function getBotSettingKeys(){
    ...
    $stmt->close();
        $referralButtonState = ($botState['referralButtonState']??"on")=="on"?$buttonValues['on']:$buttonValues['off'];
return json_encode(['inline_keyboard'=>[
        [
        ...
        [
            ['text'=>$referralButtonState,'callback_data'=>"changeBotreferralButtonState"],
            ['text'=>"زیر مجموعه گیری" ,'callback_data'=>"deltach"]
            ],       [
...
function getUserInfoKeys($userId){
    ...
    if($userCount->num_rows > 0){
        ...
        return json_encode(['inline_keyboard'=>[
            ...
            [
                ['text'=>"➕ افزایش موجودی",'callback_data'=>"userAct_inc_$userId"],
                ['text'=>"➖ کاهش موجودی",'callback_data'=>"userAct_dec_$userId"]
            ],
            [
                ['text'=>"✅ آزاد کردن کاربر",'callback_data'=>"userAct_unban_$userId"],
                ['text'=>"❌ مسدود کردن کاربر",'callback_data'=>"userAct_ban_$userId"]
            ],
            [
                ['text'=>"✉️ پیام خصوصی",'callback_data'=>"userAct_pm_$userId"]
            ],
            [
                ['text'=>"٪ درصد تخفیف کاربر",'callback_data'=>"userAct_discount_$userId"],
                ['text'=>"🧪 محدودیت اکانت تست",'callback_data'=>"userAct_test_$userId"]
            ],
            [
                ['text'=>"📜 لیست سفارش ها",'callback_data'=>"userAct_orders_$userId"],
                ['text'=>"🛑 استثنا از تایید خودکار",'callback_data'=>"userAct_exauto_$userId"]
            ],
            [
                ['text'=>"🧹 صفرکردن موجودی",'callback_data'=>"userAct_zero_$userId"]
            ],
...
function getRenameMainButtonsKeys(){
    ...
    $stmt->close();
    $keys = array();
    $keys[] = [['text'=>"✏️ تغییر متن دکمه استارت",'callback_data'=>"renameStartButton"]];
    if($buttons->num_rows > 0){
        while($row = $buttons->fetch_assoc()){
            $rowId = $row['id'];
            $title = str_replace("MAIN_BUTTONS","", $row['type']);
            $keys[] = [
                ['text'=>"✏️",'callback_data'=>"renameMainButton" . $rowId],
                ['text'=>$title??" " ,'callback_data'=>"deltach"]
            ];
        }
    }else{
    ...
    $keys[] = [['text'=>$buttonValues['back_button'],'callback_data'=>"mainMenuButtons"]];
    return json_encode(['inline_keyboard'=>$keys]);
}
function getOrderMainButtonsKeys(){
    global $connection, $buttonValues;
    $stmt = $connection->prepare("SELECT * FROM `setting` WHERE `type` LIKE '%MAIN_BUTTONS%' ORDER BY `id` ASC");
    $stmt->execute();
    $buttons = $stmt->get_result();
    $stmt->close();
    $rows = $buttons->fetch_all(MYSQLI_ASSOC);
    $keys = array();
    if(count($rows) > 0){
        foreach($rows as $i => $row){
            $rowId = $row['id'];
            $title = str_replace("MAIN_BUTTONS","", $row['type']);
            $up = ($i>0)?'moveMainButtonUp':'deltach';
            $down = ($i < count($rows)-1)?'moveMainButtonDown':'deltach';
            $keys[] = [
                ['text'=>"⬆️",'callback_data'=>$up . $rowId],
                ['text'=>$title??" ",'callback_data'=>"deltach"],
                ['text'=>"⬇️",'callback_data'=>$down . $rowId],
            ];
        }
    }else{
        $keys[] = [['text'=>"دکمه ای یافت نشد ❕",'callback_data'=>"deltach"]];
    ...
    $keys[] = [['text'=>"✏️ تغییر اسم دکمه ها",'callback_data'=>"renameMainButtons"]];
    $keys[] = [['text'=>"🔀 چینش دکمه ها",'callback_data'=>"orderMainButtons"]];
    $keys[] = [['text'=>$buttonValues['back_button'],'callback_data'=>"managePanel"]];
    return json_encode(['inline_keyboard'=>$keys]);
}
