...
function bot($method, $datas = []){
    ...
    if(isset($datas['reply_markup']) && is_array($datas['reply_markup'])){
        $datas['reply_markup'] = json_encode($datas['reply_markup'], 488);
    }

    // Do not send empty/null optional fields (Telegram can reject empty parse_mode, etc.)
    foreach(['parse_mode','caption','text'] as $opt){
        // keep text if method needs it; we don't unset text
    }
    if(isset($datas['parse_mode']) && ($datas['parse_mode'] === null || $datas['parse_mode'] === '')){
        unset($datas['parse_mode']);
    }
    if(isset($datas['caption']) && ($datas['caption'] === null)){
        unset($datas['caption']);
    }
    ...
    $res = curl_exec($ch);
    if (curl_error($ch)) {
        // avoid breaking bot output; return false on curl errors
        return false;
    } else {
