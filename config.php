...
function dbCreateSqlBackupFile($prefix = 'deltabot_backup'){
    global $dbhost, $dbname, $dbuser, $dbpass, $connection;
    @set_time_limit(300);
    ...
    if(isShellExecAvailable()){
        ...
        if($which !== '' && @is_executable($which)){
            ...
            $cmd = "$which --single-transaction --quick --skip-lock-tables $hostArg --user=".escapeshellarg($dbuser)." --password=".escapeshellarg($dbpass)." ".escapeshellarg($dbname)." > ".escapeshellarg($path)." 2>&1";
            @shell_exec($cmd);
    ...
    // 2) Fallback: stream dump via mysqli (avoid building huge strings)
    $fh = @fopen($path, 'w');
    if(!$fh) return null;
    ...
    while($trow = $tablesRes->fetch_row()){
        ...
        // Dump data row-by-row (memory safe)
        $dataRes = $connection->query("SELECT * FROM `{$table}`", MYSQLI_USE_RESULT);
        if($dataRes){
