...
// ---------------- Cart-to-Card message template helper (global)
function renderCartToCartTemplate($template, $context = []){
    // This function replaces placeholders with gateway settings and (if placeholders are missing)
    // appends the card details automatically.
    global $paymentKeys;
    if($template === null) $template = "";
    $template = (string)$template;

    $account = $paymentKeys['bankAccount'] ?? ($paymentKeys['cardNumber'] ?? ($paymentKeys['account'] ?? ""));
    $holder  = $paymentKeys['holderName'] ?? ($paymentKeys['cardHolder'] ?? ($paymentKeys['name'] ?? ""));

    // allow passing overrides
    if(isset($context['ACCOUNT-NUMBER'])) $account = $context['ACCOUNT-NUMBER'];
    if(isset($context['HOLDER-NAME'])) $holder = $context['HOLDER-NAME'];

    $hasAccountToken = (strpos($template, "ACCOUNT-NUMBER") !== false);
    $hasHolderToken  = (strpos($template, "HOLDER-NAME") !== false);

    if($account !== "" || $holder !== ""){
        $template = str_replace(["ACCOUNT-NUMBER","HOLDER-NAME"], [$account, $holder], $template);

        // If admin removed tokens from the template, still show card details automatically
        if(!$hasAccountToken && !$hasHolderToken && trim($account) !== ""){
            $safeHolder = htmlspecialchars((string)$holder, ENT_QUOTES, 'UTF-8');
            $safeAcc    = htmlspecialchars((string)$account, ENT_QUOTES, 'UTF-8');
            $template  .= "\n\n💳 شماره کارت: <code>{$safeAcc}</code>";
            if(trim($holder) !== "") $template .= "\n👤 به نام: {$safeHolder}";
        }
    }
...
// If admin set a global Cart-to-Card message, apply it everywhere
if(isset($mainValues['cart_to_cart_message']) && trim((string)$mainValues['cart_to_cart_message']) !== ""){
    $mainValues['buy_account_cart_to_cart'] = $mainValues['cart_to_cart_message'];
    $mainValues['increase_wallet_cart_to_cart'] = $mainValues['cart_to_cart_message'];
    $mainValues['renew_ccount_cart_to_cart'] = $mainValues['cart_to_cart_message'];
}
...
function getMainKeys(){
    ...
    // --- Custom MAIN_BUTTONS (apply saved order + columns)
...
function getRenameButtonsKeys($page=0){
    global $buttonValues, $mainValues;
    $keys = array();
    ...
    // message templates to rename (MAINVALUE_* keys)
    $messageKeys = [
        "cart_to_cart_message" => "پیام کارت به کارت (همه بخش‌ها)",
        "buy_account_cart_to_cart" => "پیام کارت به کارت (خرید)",
        "increase_wallet_cart_to_cart" => "پیام کارت به کارت (شارژ کیف پول)",
        "renew_ccount_cart_to_cart" => "پیام کارت به کارت (تمدید)",
    ];
    foreach(array_keys($messageKeys) as $mk){
        if(isset($mainValues[$mk])) $btnKeys[] = $mk;
    }

    // bring important items closer to top (easier to find)
    $important = ["cart_to_cart_message","buy_account_cart_to_cart","increase_wallet_cart_to_cart","renew_ccount_cart_to_cart","cart_to_cart"];
...
function getArrangeButtonsMenuKeys(){
    ...
    $keys = [
        ...
        [['text'=>"↕️ چینش دکمه‌های سفارشی", 'callback_data'=>"arrangeCustomButtons"]],
