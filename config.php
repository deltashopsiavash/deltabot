...
function getAdminChatIds(): array {
    ...
    $ids = [];
    if(isset($admin) && is_numeric($admin)) $ids[] = (int)$admin;

    // users with isAdmin=1
    $res = $connection->query("SELECT `userid` FROM `users` WHERE `isAdmin` = 1");
    if($res){
        while($row = $res->fetch_assoc()){
            if(is_numeric($row['userid'])) $ids[] = (int)$row['userid'];
        }
    }
...
/**
 * Split bank accounts by newline / comma and return a random one.
 */
function pickRandomBankAccount($raw){
    if($raw === null) return null;
    $raw = trim((string)$raw);
    if($raw === "") return "";
    $parts = preg_split("/[\n,]+/u", $raw);
    $parts = array_values(array_filter(array_map('trim', $parts), fn($v)=>$v!==""));
    if(count($parts) <= 1) return $raw;
    return $parts[random_int(0, count($parts)-1)];
}

function sendPhotoToAdmins($photo, $caption = null, $keyboard = null, $parse = "HTML"){
    $firstRes = null;
    foreach(getAdminChatIds() as $aid){
...
function sendMessageToAdmins($txt, $key = null, $parse = "HTML"){
    $firstRes = null;
    foreach(getAdminChatIds() as $aid){
        $res = sendMessage($txt, $key, $parse, $aid);
        if($firstRes === null) $firstRes = $res;
    }
    return $firstRes;
}
...
/**
 * User temp flags stored as JSON in users.temp
 */
function getUserTempFlags(int $userId): array {
    global $connection;
...
function setUserTempFlags(int $userId, array $flags): void {
    global $connection;
    $temp = json_encode($flags, JSON_UNESCAPED_UNICODE);
    $stmt = $connection->prepare("UPDATE `users` SET `temp` = ? WHERE `userid` = ?");
    $stmt->bind_param("si", $temp, $userId);
    $stmt->execute();
    $stmt->close();
}
function toggleUserTempFlag(int $userId, string $flagKey): bool {
    $flags = getUserTempFlags($userId);
    $new = !(!empty($flags[$flagKey]));
    $flags[$flagKey] = $new ? 1 : 0;
    setUserTempFlags($userId, $flags);
    return $new;
}
...
function getBotSettingKeys(){
    ...
    $inviteBtn = ($botState['inviteState']??"on")=="on"?$buttonValues['on']:$buttonValues['off'];
    ...
    return json_encode(['inline_keyboard'=>[
        ...
        [
            ['text'=>$inviteBtn,'callback_data'=>"changeBotinviteState"],
            ['text'=>"دکمه زیرمجموعه‌گیری",'callback_data'=>"deltach"]
...
function getUserInfoKeys($userId){
    ...
    if($userCount->num_rows > 0){
        ...
        return json_encode(['inline_keyboard'=>[
            ...
            [
                ['text'=>"➕ افزایش موجودی",'callback_data'=>"uInc$userId"],
                ['text'=>"➖ کاهش موجودی",'callback_data'=>"uDec$userId"]
            ],
            [
            ...
            [
                ...
                ['text'=>"🧾 لیست سفارش‌ها",'callback_data'=>"uOrders$userId"]
            ],
            [
                ['text'=>"🎯 درصد تخفیف",'callback_data'=>"uDisc$userId"],
                ['text'=>"🧪 محدودیت تست",'callback_data'=>"uTest$userId"]
            ],
            [
                ['text'=>"⚡️ استثنا از تایید خودکار",'callback_data'=>"uExAuto$userId"],
                ['text'=>"🧹 صفر کردن موجودی",'callback_data'=>"uZero$userId"]
            ],
