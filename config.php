...
function applyUserPercentDiscount($userId, $amount){
    $d = (int)(getSettingValue("USER_DISCOUNT_" . $userId, 0));
    if($d < 0) $d = 0;
    if($d > 100) $d = 100;
    if($d == 0) return (int)$amount;
    return (int)floor(((int)$amount) * (100 - $d) / 100);
}

function isUserNoAutoApprove($userId){
    return (string)getSettingValue("USER_NO_AUTOAPPROVE_" . $userId, "0") === "1";
}
