...
if($list->num_rows > 0){
    ...
    $text = $info['text'];
    $pin = isset($info['pin'])?(int)$info['pin']:0;
    ...
    if($usersList->num_rows > 0) {
        while($user = $usersList->fetch_assoc()){
            if($type == 'text'){
                $sent = sendMessage($text,$keys,null,$user['userid']);
                if($pin==1 && isset($sent->result->message_id)){
                    bot('pinChatMessage',['chat_id'=>$user['userid'],'message_id'=>$sent->result->message_id,'disable_notification'=>true]);
                }
            }elseif($type == 'music'){
                $sent = bot('sendAudio',[
                    'chat_id' => $user['userid'],
