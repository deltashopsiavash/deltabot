...
if($list->num_rows > 0){
    ...
    if($usersList->num_rows > 0) {
        while($user = $usersList->fetch_assoc()){
            if($type == 'text'){
                    $sent = bot('sendMessage',['chat_id'=>$user['userid'],'text'=>$text,'reply_markup'=>$keys]);
                if($pin==1 && isset($sent->result->message_id)) bot('pinChatMessage',['chat_id'=>$user['userid'],'message_id'=>$sent->result->message_id]);
            }elseif($type == 'music'){
                $sent = bot('sendAudio',[
                    'chat_id' => $user['userid'],
                    'audio' => $file_id,
