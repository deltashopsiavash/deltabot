...
if($botState['cartToCartAutoAcceptState']=="on"){
    ...
    while($payInfo = $info->fetch_assoc()){
        ...
        // user exception from cart-to-cart auto accept
        $tempArr = json_decode($userinfo['temp']??null,true);
        if(is_array($tempArr) && isset($tempArr['exclude_cart_auto']) && $tempArr['exclude_cart_auto'] == 1) continue;
